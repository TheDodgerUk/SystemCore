...
namespace Photon.Pun
{
    ...
    public struct InstantiateParameters
    {
        ...
        public InstantiateParameters(GameObject gameObject, Vector3 position, Quaternion rotation, byte @group, object[] data, byte objLevelPrefix, int[] viewIDs, Player creator, int timestamp)
        {
    ...
    public static partial class PhotonNetwork
    {
        ...
        public static GameObject Setup(GameObject gameObject, Vector3 position, Quaternion rotation, byte group = 0, object[] data = null)
        {
            if (CurrentRoom == null)
            {
                Debug.LogError("Can not Instantiate before the client joined/created a room. State: " + PhotonNetwork.NetworkClientState);
            ...
            Pun.InstantiateParameters netParams = new InstantiateParameters(gameObject, position, rotation, group, data, currentLevelPrefix, null, LocalPlayer, ServerTimestamp);
            return NetworkInstantiate(netParams, false);
        }
        ...
        private static GameObject NetworkInstantiate(Pun.InstantiateParameters parameters, bool roomObject = false, bool instantiateEvent = false)
        {
            ...
            if(string.IsNullOrEmpty(parameters.prefabName) == true)
            {
                return null;
            }

            if (parameters.gameObject == null)
            {
                go = prefabPool.Instantiate(parameters.prefabName, parameters.position, parameters.rotation);
            }
            else
            {
                go = parameters.gameObject;
            }
