...
public class RopeControllerSimple : MonoBehaviour
{
    ...
    private float minRopeLength = 4f;
    ...
    private Vector3 m_ObjectFromPostion;
    private Vector3 m_ObjectTooPostion;

    public void SetRopeLength(float length, float width)
    {
        ropeLength = length;
        minRopeLength = length;
    ...
    void Start()
    {
        var rig = whatTheRopeIsConnectedTo.gameObject.ForceComponent<Rigidbody>();
        rig.isKinematic = true;
        springJoint = whatTheRopeIsConnectedTo.gameObject.ForceComponent<SpringJoint>();

        //Init the line renderer we use to display the rope
        lineRenderer = whatTheRopeIsConnectedTo.gameObject.ForceComponent<LineRenderer>();
        m_Material = Resources.Load<Material>("Materials/black");
    ...
    void Update()
    {
        if (springJoint != null && lineRenderer != null)
        {
            //Add more/less rope
            bool updated = UpdateWinch();
            // optimzation
            updated = (updated) || (m_ObjectFromPostion != whatIsHangingFromTheRope.position) || (m_ObjectTooPostion != whatTheRopeIsConnectedTo.position);

            m_ObjectFromPostion = whatIsHangingFromTheRope.position;
            m_ObjectTooPostion = whatTheRopeIsConnectedTo.position;
            if (updated == true)
            {
