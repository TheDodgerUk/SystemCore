...
namespace EPOOutline
{
    ...
    {
        private static int m_sRenderCount = 0;
...
#pragma warning disable CS0649
            ...
            public Outliner.EnumItemSelected ItemSelected = Outliner.EnumItemSelected.Valid; 
            public Color Color => Outliner.Instance.GetLineColor(ItemSelected);
...
#pragma warning disable CS0649
        ...
        public List<Renderer> RenderersRef { get; private set; }
...
#pragma warning restore CS0649
        ...
        {
            AddAllChildRenderersToRenderingList(RenderersAddingMode.All);
        }

        public void InitialiseActiveRenderers()
        {
            AddAllChildActiveRenderersToRenderingList(RenderersAddingMode.All);
        }

        public void Initialise(GameObject obj)
        {
            AddAllChildRenderersToRenderingList(obj, RenderersAddingMode.All);
        }
        ...
        public void SetOutLine(bool enable) => this.enabled = enable;
        public Outliner.EnumItemSelected ItemSelected
        {
            get
            {
                return outlineParameters.ItemSelected;
            }
            set
            {
                outlineParameters.ItemSelected = value;
            }
        }
        ...
        public void AddAllChildRenderersToRenderingList(RenderersAddingMode renderersAddingMode = RenderersAddingMode.All)
        {
            ...
            RenderersRef = GetComponentsInChildren<Renderer>(true).ToList();
            if (RenderersRef.Count > GlobalConsts.MAX_OUTLINE_RENDERERS)
            {
                Debug.LogError($"Too many renderers, it locks the system, Count  {RenderersRef}, m_sRenderCount {m_sRenderCount}", this.gameObject);
            }
            else
            {
                m_sRenderCount += RenderersRef.Count;
                foreach (var renderer in RenderersRef)
                {
        ...
        public void AddAllChildActiveRenderersToRenderingList(RenderersAddingMode renderersAddingMode = RenderersAddingMode.All)
        {
            outlineTargets.Clear();
            RenderersRef = GetComponentsInChildren<Renderer>(true).ToList();
            RenderersRef.RemoveAll(e => e.enabled == false);
            RenderersRef.RemoveAll(e => e.name.EndsWith("LOD1"));
            RenderersRef.RemoveAll(e => e.name.EndsWith("LOD2"));
            if (RenderersRef.Count > GlobalConsts.MAX_OUTLINE_RENDERERS)
            {
                Debug.LogError($"Too many renderers, it locks the system, Count:  {RenderersRef.Count},", this.gameObject);
            }
            else
            {
                foreach (var renderer in RenderersRef)
        ...
        public void AddAllChildRenderersToRenderingList(GameObject obj, RenderersAddingMode renderersAddingMode = RenderersAddingMode.All)
        {
            outlineTargets.Clear();
            if (obj != null)
            {
                RenderersRef = obj.GetComponentsInChildren<Renderer>(true).ToList();
                foreach (var renderer in RenderersRef)
                {
                    if (!MatchingMode(renderer, renderersAddingMode))
                        continue;
