...
public static class ObjLoader
{
    ...
    [MenuItem("GameObject/Export From OBJ")]
    static void ObjExportMenu()
    {
        var go = Selection.activeGameObject;
        string path = AssetDatabase.GetAssetPath(go);
        path = Path.ChangeExtension(path, ".obj");
        path = AssetDatabase.GenerateUniqueAssetPath(path);
        var f = go.GetComponentInChildren<MeshFilter>();
        ObjExporter.MeshToFile(path, f);
    }

    [MenuItem("GameObject/Export To OBJ")]
    static void ExportToObj()
    {
        var r = Selection.activeGameObject.GetComponentInChildren<MeshFilter>();
        string path = Application.dataPath.Replace("Assets", "Assets/OBJ.obj");
        File.WriteAllText(path, ObjExporter.MeshToString(r));
    ...
    public static Material[] LoadMTLFile(string fn)
    {
        ...
        foreach (string ln in File.ReadAllLines(fn))
        {
            ...
            if (cmps[0] == "newmtl")
            {
                ...
                currentMaterial = new Material(Shader.Find(ShaderName)) { name = data };
            }
            else if (cmps[0] == "Kd")
            {
    ...
    public static GameObject LoadOBJFile(string fn)
    {
        ...
        foreach (string obj in objectNames)
        {
            ...
            for (int i = 0; i < meshMaterialNames.Count; i++)
            {
                ...
                {
                    processedMaterials[i] = new Material(Shader.Find(ShaderName));
                }
                else
                {
