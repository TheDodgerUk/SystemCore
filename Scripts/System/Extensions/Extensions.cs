...
public static class Extensions
{
    ...
    public static List<List<T>> SplitList<T>(this List<T> locations, int nSize)
    {
    ...
    public static List<T> Move<T>(this List<T> list, int startIndex, int AddIndex)
    {
    ...
    public static int LastIndex(this Array array) => (array.Length - 1);
    ...
    public static T GetNextBounce<T>(this List<T> list, ref int index, ref ListBounce bounce)
    {
        if((list.Count -1) == index && bounce == ListBounce.Down)
        {
            bounce = ListBounce.Up;
        }
        if (index == 0 && bounce == ListBounce.Up)
        {
            bounce = ListBounce.Down;
        }
        if(bounce == ListBounce.Up)
        {
            index--;
        }
        else
    ...
    public static Tuple<TKey, TValue> GetAny<TKey, TValue>(this Dictionary<TKey, TValue> dictionary)
    {
        if (dictionary == null || dictionary.Count == 0)
        {
            return null;
        }
        return dictionary.FirstOrDefault().AsTuple();
    }
    ...
    public static List<Tuple<T1, T2>> ExtractAsTuples<T1, T2>(this Dictionary<T1, T2> dict)
    {
        var keys = dict.Keys.ToList();
        return keys.Extract(k => Tuple.New(k, dict[k]));
    }
    ...
    public static T GetRandom<T>(this List<T> list)
    {
        ...
        {
            return list[Utils.Unity.GetRandom(0, list.Count)];
        }
        return default(T);
    }
    ...
    {
        return Utils.Unity.GetRandom(0, self.Count);
    }

    public static List<T> Randomise<T>(this List<T> self)
    {
        Random rand = new Random();
        var shuffled = self.OrderBy(_ => rand.Next()).ToList();
        return shuffled;
    }
    ...
    public static string[] GetNumberStrings(this string str)
    {
        List<string> numbersList = new List<string>();
        string[] numbers = System.Text.RegularExpressions.Regex.Split(str, @"\D+");
    ...
    public static bool GetNumber(this string str, out int number)
    {
        bool returnValue = Int32.TryParse(str.GetNumberString(), out number);
        return returnValue;
    }
    ...
    public static string ToRealNumbers(this byte[] bytes)
    {
        ...
        { "Bytes", "KB", "MB", "GB", "TB", "PB" };
            ...
            int counter = 0;
            decimal number = (decimal)bytes.Length;
    ...
    {
        return $"{((int)t.TotalHours).ToString("00")}:{((int)t.Minutes).ToString("00")}:{((int)t.Seconds).ToString("00")}";
    }
}
