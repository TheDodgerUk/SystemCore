...
public static class MathsExtensions
{
    ...
    public static bool Contains(this Vector2 v, float value)
    {
    ...
    public static void Toggle(ref this bool toggle) => toggle = !toggle;
    ...
    public static T WrapEnum<T>(this T value, int delta) where T : struct
    {
        var values = Utils.Code.GetEnumValues<T>();
        int index = values.IndexOf(value) + delta;
        return values[index.Wrap(values)];
    }

    public static T WrapIncrementEnum<T>(this T value) where T : struct
    {
        var values = Utils.Code.GetEnumValues<T>();
        int index = values.IndexOf(value);
        return values[index.WrapIncrement(values)];
    }

    public static T WrapDecrementEnum<T>(this T value) where T : struct
    {
        var values = Utils.Code.GetEnumValues<T>();
        int index = values.IndexOf(value);
        return values[index.WrapDecrement(values)];
    }



    public static int WrapIncrement(this int index, IList list) { index++; return index.Wrap(list.Count); }
    public static int WrapDecrement(this int index, IList list) { index--; return index.Wrap(list.Count); }

    public static int WrapIncrement(this int index, int count) { index++; return index.Wrap(count); }
    public static int WrapDecrement(this int index, int count) { index--; return index.Wrap(count); }

    public static int ClampIncrement(this int index, int count) => (index + 1).Clamp(0, count);
    public static int ClampDecrement(this int index, int count) => (index - 1).Clamp(0, count);

    public static int ClampIncrement(this int index, IList list) => (index + 1).Clamp(0, list.Count - 1);
    public static int ClampDecrement(this int index, IList list) => (index - 1).Clamp(0, list.Count - 1);

    public enum ClampWrap
    {
        Clamp,
        Wrap,
    }

    public static int Decrement(this int index, IList list, ClampWrap clampWrap)
    {
        if (clampWrap == ClampWrap.Clamp)
        {
            return index.ClampDecrement(list);
        }
        else
        {
            return index.WrapDecrement(list);
        }
    }

    public static int Increment(this int index, IList list, ClampWrap clampWrap)
    {
        if (clampWrap == ClampWrap.Clamp)
        {
            return index.ClampIncrement(list);
        }
        else
        {
            return index.WrapIncrement(list);
        }
    }
    ...
    public static Vector2 ToVector2(this Complex32 c) => new Vector2(c.Real, c.Imaginary);

    public static double ToDecibels(this Complex32 c) => LinearToDecibels(Complex32.Abs(c));
    ...
    // 0 - 1
    public static float Percentage(this float amount, float min, float max)
    {
        return Mathf.InverseLerp(min, max, amount);
    }
    ...
    public static float GetAxis(this Vector3 vect3, Axis axis)
    {
        switch (axis)
        {
            case Axis.X: return vect3.x;
            case Axis.Y: return vect3.y;
            case Axis.Z: return vect3.z;
        }
        return 0;
    }

    public static Vector3 SetAxis(this Vector3 vect3, Axis axis, float amount)
    {
        switch (axis)
        {
            case Axis.X: vect3.x = amount; break;
            case Axis.Y: vect3.y = amount; break;
            case Axis.Z: vect3.z = amount; break;
        }
        return vect3;
    }

    public static Vector3 CreateVector3Axis(this Axis axis, Vector3 data)
    {
        switch (axis)
        {
            case Axis.X: return new Vector3(data.x, 0, 0);
            case Axis.Y: return new Vector3(0, data.y, 0);
            case Axis.Z: return new Vector3(0, 0, data.z);
        }
        return Vector3.zero;
    }

    public static Vector3 CreateVector3Axis(this Vector3 data, Axis axis)
    {
        switch (axis)
        {
            case Axis.X: return new Vector3(data.x, 0, 0);
    ...
    public static Vector3 CreateVector3AxisAbs(this Axis axis, Vector3 data)
    {
        switch (axis)
        {
            case Axis.X: return new Vector3(Mathf.Abs(data.x), 0, 0);
            case Axis.Y: return new Vector3(0, Mathf.Abs(data.y), 0);
            case Axis.Z: return new Vector3(0, 0, Mathf.Abs(data.z));
        }
        return Vector3.zero;
    }

    public static Vector3 CreateVector3AxisAbs(this Vector3 data, Axis axis)
    {
        switch (axis)
        {
            case Axis.X: return new Vector3(Mathf.Abs(data.x), 0, 0);
