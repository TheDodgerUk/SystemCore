...
Shader "ASESampleShaders/Fake Interiors FREE"
{
	Properties
	{
		...
		_PositionProps("Position Props, XY", Vector) = (0,0,0,0)
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			#endif
			float2 appendResult396 = (float2(( staticSwitch482 * _PropsTexTiling + _PositionProps.x ).x , ( staticSwitch482 * _PropsTexTiling + _PositionProps.y ).y));
			float2 appendResult546 = (float2(( staticSwitch482 * _PropsTexTiling + _PositionProps.y ).y , ( staticSwitch482 * _PropsTexTiling + _PositionProps.x).x));
			#ifdef _SWITCHPLANE_ON
