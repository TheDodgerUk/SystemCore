...
Shader "Shader Forge/HologramFX" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _FXMask; uniform float4 _FXMask_ST;
...
////// Emissive:
                ...
                float2 node_6564 = (_MaskTiling*i.uv0);
                float4 _FXMask_var = tex2D(_FXMask,TRANSFORM_TEX(node_6564, _FXMask));
                float DetailMask = _FXMask_var.r;
                float3 emissive = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel)*_FresnelColor.rgb)+(_Emission*(lerp(_OffColor.rgb,_HologramColor.rgb,node_9455)*node_9455*saturate((1.0 - smoothstep( 0.0, DetailMask, node_9455 ))))));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _FXMask_var = tex2D(_FXMask,TRANSFORM_TEX(node_6564, _FXMask));
                float DetailMask = _FXMask_var.r;
                o.Emission = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),_Fresnel)*_FresnelColor.rgb)+(_Emission*(lerp(_OffColor.rgb,_HologramColor.rgb,node_9455)*node_9455*saturate((1.0 - smoothstep( 0.0, DetailMask, node_9455 ))))));
