    ...
    public static class AssetLoader
    {
        ...
        private static void SetupHumanoidAvatar(AssetLoaderContext assetLoaderContext, Animator animator)
        {
            ...
            if (mapping.Count > 0)
            {
                ...
                {
                    var backupTransforms = new Dictionary<Transform, TransformBackup>();
                    var skeletonBones = new Dictionary<Transform, SkeletonBone>(assetLoaderContext.GameObjects.Count);
                    foreach (var kvp in assetLoaderContext.GameObjects)
                    {
                        if (kvp.Key.IsBone)
                        {
                            backupTransforms[kvp.Value.transform] = new TransformBackup(kvp.Value.transform);
                            skeletonBones.Add(kvp.Value.transform, CreateSkeletonBone(kvp.Value.transform));
                            kvp.Value.transform.position -= hipsTransform.position;
                            kvp.Value.transform.rotation = Quaternion.Inverse(hipsTransform.rotation) * kvp.Value.transform.rotation;
                        }
                    }
                    var parentTransform = hipsTransform.parent;
                    while (parentTransform != null)
                    {
                        if (!skeletonBones.ContainsKey(parentTransform))
                        {
                            skeletonBones.Add(parentTransform, CreateSkeletonBone(parentTransform));
                        }
                    ...
                    avatar.name = $"{assetLoaderContext.RootGameObject.name}Avatar";
                    animator.avatar = avatar;
                    foreach (var kvp in backupTransforms)
                    {
